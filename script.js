...
async function loadWeather() {
    ...
    try {
        // Показываем демо данные для 1 августа 2025 года
        // В реальном проекте здесь можно использовать API для долгосрочного прогноза
        showWeddingDayWeather();
        
    } catch (error) {
        console.error('Weather loading error:', error);
        showWeddingDayWeather(); // Показываем демо данные при ошибке
    }
}

function showWeddingDayWeather() {
    // Прогноз погоды на 1 августа 2025 года (демо данные)
    ...
    displayWeather(weddingDayData, true);
}

function displayWeather(data, isWeddingDay = false) {
    const weatherLoading = document.getElementById('weather-loading');
    const weatherData = document.getElementById('weather-data');
    ...
    if (isWeddingDay) {
        // Добавляем пометку о прогнозе на день свадьбы
        const weddingNote = document.createElement('div');
        weddingNote.style.cssText = 'margin-top: 15px; padding: 15px; background: rgba(255, 193, 7, 0.15); border-radius: 12px; font-size: 0.95rem; color: #856404; border-left: 4px solid #ffc107;';
        weddingNote.innerHTML = '📅 <strong>Прогноз на 1 августа 2025 года</strong><br>Данные основаны на климатических условиях региона для этого времени года.';
        weatherData.appendChild(weddingNote);
    }
}
...
// Function to show the personalized modal
function showPersonalizedModal(guestName) {
    const modal = document.getElementById("personalized-modal");
    const modalTitle = document.getElementById("personalized-modal-title");
    const modalText = document.getElementById("personalized-modal-text");
    const closeBtn = modal.querySelector(".modal-close");

    if (guestName) {
        modalTitle.textContent = `Дорогоие ${guestName}!`;
...
// Function to close the personalized modal
...
// Check for guest parameter on page load
document.addEventListener("DOMContentLoaded", function() {
